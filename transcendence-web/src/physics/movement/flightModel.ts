...
/**
 * Pure flight physics update.
 * Returns a new state without mutating inputs.
 */
export function computeFlightUpdate(
  ...
  if (dt <= 0) {
    return {
      newVelocity: { x: currentVelocity.x, y: currentVelocity.y },
  ...
  if (flightAssistEnabled) {
    const faAcceleration = computeFlightAssist(currentVelocity, newRotation, config, dt);
    ax += faAcceleration.x;
    ay += faAcceleration.y;
  }
...
/**
 * Flight Assist damps lateral and backward movement.
 * It does not damp forward velocity.
 */
