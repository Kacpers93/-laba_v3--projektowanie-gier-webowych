...
export class PlayerShipEntity extends WorldEntity {
  ...
  public constructor(config: {
    ...
    this.flightConfig = {
      ...
      softDragCoefficient: Math.abs(config.flightConfig.softDragCoefficient),
      flightAssistDeadzone: Math.abs(config.flightConfig.flightAssistDeadzone),
    };
  }
  ...
  public updateFlight(dt: number, input: FlightInput): void {
    ...
    this.velocity = {
      x: result.newVelocity.x,
      y: result.newVelocity.y,
    };
    this.rotation = result.newRotation;
    this.currentAcceleration = {
      x: result.acceleration.x,
